...
struct IsDecomposable<phmap::void_t<decltype(Policy::apply(RequireUsableKey<typename Policy::key_type, Hash, Eq>(), std::declval<Ts>()...))>, Policy,
...
// --------------------------------------------------------------------------
template <typename T> uint32_t LeadingZeros(T x)
{
    PHMAP_IF_CONSTEXPR(sizeof(T) == 8)
...
class parallel_hash_set
{
  ...
  public:
    ...
    template <class F> void for_each_m(F &&fCallback)
    {
        for (auto &inner : sets_)
        {
            typename Lockable::UniqueLock m(inner);
...
#if __cplusplus >= 201703L
    template <class ExecutionPolicy, class F> void for_each(ExecutionPolicy &&policy, F &&fCallback) const
    {
        std::for_each(std::forward<ExecutionPolicy>(policy), sets_.begin(), sets_.end(),
                      [&](auto const &inner)
                      {
                          typename Lockable::SharedLock m(const_cast<Inner &>(inner));
                          std::for_each(inner.set_.begin(), inner.set_.end(), fCallback);
    ...
    template <class ExecutionPolicy, class F> void for_each_m(ExecutionPolicy &&policy, F &&fCallback)
    {
        std::for_each(std::forward<ExecutionPolicy>(policy), sets_.begin(), sets_.end(),
                      [&](auto &inner)
                      {
    ...
    //        for (auto& p : set) { ...; }});
    // -------------------------------------------------
    template <class F> void with_submap(size_t idx, F &&fCallback) const
    {
    ...
    template <class F> void with_submap_m(size_t idx, F &&fCallback)
    {
  ...
  private:
    ...
    // TODO(alkis): Optimize this assuming *this and that don't overlap.
    // --------------------------------------------------------------------
    template <class Mtx2_> parallel_hash_set &move_assign(parallel_hash_set<N, RefSet, Mtx2_, Policy, Hash, Eq, Alloc> &&that, std::true_type)
    {
    ...
    template <class Mtx2_> parallel_hash_set &move_assign(parallel_hash_set<N, RefSet, Mtx2_, Policy, Hash, Eq, Alloc> &&that, std::false_type)
    {
