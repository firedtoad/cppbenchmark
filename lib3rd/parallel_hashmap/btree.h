...
template <class T> struct IsTriviallyMoveAssignableReference : std::false_type
{
};
...
template <typename T> struct is_trivially_copyable : std::integral_constant<bool, type_traits_internal::is_trivially_copyable_impl<T>::kValue>
{
};
} // namespace type_traits_internal
...
template <class T> struct IsSwappable : phmap::type_traits_internal::is_detected<IsSwappableImpl, T>
{
};

template <class T> struct IsNothrowSwappable : phmap::type_traits_internal::is_detected<IsNothrowSwappableImpl, T>
{
};

template <class T, phmap::enable_if_t<IsSwappable<T>::value, int> = 0> void Swap(T &lhs, T &rhs) noexcept(IsNothrowSwappable<T>::value)
{
    swap(lhs, rhs);
}
...
#define PHMAP_COMPARE_INLINE_INIT(type, name, init) template <typename T> const T compare_internal::type##_base<T>::name(init)
...
template <typename Key, typename Compare, typename Alloc, int TargetNodeSize, bool Multi, typename SlotPolicy> struct common_params
{
...
template <typename Key> struct set_slot_policy
{
    ...
    template <typename Alloc> static void swap(Alloc * /*alloc*/, slot_type *a, slot_type *b)
    {
        using std::swap;
        swap(*a, *b);
    }

    template <typename Alloc> static void move(Alloc * /*alloc*/, slot_type *src, slot_type *dest)
    {
        *dest = std::move(*src);
    }

    template <typename Alloc> static void move(Alloc *alloc, slot_type *first, slot_type *last, slot_type *result)
    {
...
template <typename Params> class btree_node
{
  ...
  public:
    ...
    using use_linear_search =
        std::integral_constant<bool, std::is_arithmetic<key_type>::value && (std::is_same<phmap::Less<key_type>, key_compare>::value ||
...
#pragma GCC diagnostic pop
    ...
    template <typename K> SearchResult<int, is_key_compare_to::value> lower_bound(const K &k, const key_compare &comp) const
    {
...
template <typename Params> class btree
{
  ...
  public:
    ...
    // Requirement: if `key` already exists in the btree, does not consume `args`.
    // Requirement: `key` is never referenced after consuming `args`.
    template <typename... Args> std::pair<iterator, bool> insert_hint_unique(iterator position, const key_type &key, Args &&...args);
  ...
  private:
    ...
    template <typename K> SearchResult<iterator, false> internal_locate_impl(const K &key, std::false_type /* IsCompareTo */) const;
...
////
// btree_node methods
template <typename P> template <typename... Args> inline void btree_node<P>::emplace_value(const size_type i, allocator_type *alloc, Args &&...args)
{
    assert(i <= count());
...
template <typename P> inline void btree_node<P>::remove_values_ignore_children(int i, size_type to_erase, allocator_type *alloc)
{
...
template <typename P> void btree_node<P>::rebalance_right_to_left(const int to_move, btree_node *right, allocator_type *alloc)
{
...
template <typename P> void btree_node<P>::rebalance_left_to_right(const int to_move, btree_node *right, allocator_type *alloc)
{
    assert(parent() == right->parent());
    assert(position() + 1 == right->position());
...
template <typename P> btree<P>::btree(const btree &x) : btree(x.key_comp(), x.allocator())
{
    copy_or_move_values_in_order(&x);
}

template <typename P> template <typename... Args> auto btree<P>::insert_unique(const key_type &key, Args &&...args) -> std::pair<iterator, bool>
{
    if (empty())
    {
...
template <typename P> template <typename InputIterator> void btree<P>::insert_iterator_unique(InputIterator b, InputIterator e)
{
    for (; b != e; ++b)
    {
...
template <typename P> auto btree<P>::erase(iterator _begin, iterator _end) -> std::pair<size_type, iterator>
{
...
template <typename P> void btree<P>::erase_same_node(iterator _begin, iterator _end)
{
...
template <typename P> auto btree<P>::erase_from_leaf_node(iterator _begin, size_type to_erase) -> iterator
{
    node_type *node = _begin.node;
    assert(node->leaf());
...
template <typename Tree> class btree_container
{
  ...
  protected:
    ...
    // transparent case.
    template <class K> using key_arg = typename KeyArg<IsTransparent<typename Tree::key_compare>::value>::template type<K, typename Tree::key_type>;
  ...
  public:
    ...
    template <typename K = key_type> const_iterator find(const key_arg<K> &key) const
    {
        return tree_.find(key);
    }
...
// ----------------------------------------------------------------------
template <typename K, typename V, typename C, typename A, typename Pred> void erase_if(btree_map<K, V, C, A> &map, Pred pred)
{
...
// Erases all elements that satisfy the predicate pred from the container.
// ----------------------------------------------------------------------
template <typename K, typename V, typename C, typename A, typename Pred> void erase_if(btree_multimap<K, V, C, A> &map, Pred pred)
{
    for (auto it = map.begin(); it != map.end();)
    {
        if (pred(*it))
