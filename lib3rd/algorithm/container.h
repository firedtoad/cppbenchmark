...
#ifndef ALGORITHM_CONTAINER_H_
#define ALGORITHM_CONTAINER_H_
...
container_algorithm_internal::decay_t<Function> c_for_each(C&& c,
                                                           Function&& f) {
...
// c_remove()
//
// Container-based version of the <algorithm> `std::erase()` function to
...
container_algorithm_internal::ContainerIter<C> c_remove(C&& c, T&& value) {
  return std::remove(container_algorithm_internal::c_begin(std::forward<C>(c)),
                     container_algorithm_internal::c_end(std::forward<C>(c)),
                     std::forward<T>(value));
}
...
template <typename C, typename Pred>
container_algorithm_internal::ContainerIter<C> c_remove_if(C&& c, Pred&& pred) {
  return std::remove_if(
      container_algorithm_internal::c_begin(std::forward<C>(c)),
      container_algorithm_internal::c_end(std::forward<C>(c)),
      std::forward<Pred>(pred));
}
...
// c_erase()
//
// Container-based version of the c++20 <algorithm> `std::erase()` function

template <typename C, typename T>
container_algorithm_internal::ContainerDifferenceType<const C> c_erase(
    C&& c, T&& value) {
  auto it = c_remove(std::forward<C>(c), std::forward<T>(value));
  auto num = std::distance(it, c.end());
...
// c_erase_if()
//
// Container-based version of the c++20 <algorithm>  `std::erase_if()` function

template <typename C, typename Pred>
container_algorithm_internal::ContainerDifferenceType<const C> c_erase_if(
    C&& c, Pred&& pred) {
  auto it = c_remove_if(std::forward<C>(c), std::forward<Pred>(pred));
  auto num = std::distance(it, c.end());
  c.erase(it, container_algorithm_internal::c_end(c));
  return num;
}
...
container_algorithm_internal::ContainerDifferenceType<const M> c_erase_nodes_if(
  ...
  return init_size - c.size();
}

// c_erase_elements_if()
//
// Map/Set Like Container-based version of of the c++20 <algorithm>
// `std::erase_if()` function
...
c_erase_elements_if(M&& c, Pred&& pred) {
  ...
  for (auto it = c.begin(); it != c.end();) {
    if (pred(*it)) {
      if constexpr (!LinearBased) {
        it = c.erase(it);
      } else {
        it = c.unordered_erase(it);
      }
    } else {
      ++it;
    }
...
// c_delete_subrange_if()

template <typename C, typename Pred>
container_algorithm_internal::ContainerDifferenceType<const C>
c_delete_subrange_if(C&& c, Pred&& pred) {
  auto it = c_remove_if(std::forward<C>(c), std::forward<Pred>(pred));
  if (it != c.end()) {
    auto start = std::distance(c.begin(), it);
    auto num = std::distance(it, c.end());
    c.DeleteSubrange(start, num);
    return num;
  }
  return 0;
}
...
// c_unique()
template <typename C>
container_algorithm_internal::ContainerIter<C> c_unique(C&& c) {
  return std::unique(container_algorithm_internal::c_begin(c),
                     container_algorithm_internal::c_end(c));
}
...
//
template <typename C>
container_algorithm_internal::ContainerDifferenceType<const C> c_erase_unique(
    C&& c) {
  auto it = c_unique(std::forward<C>(c));
  auto num = std::distance(it, c.end());
  c.erase(it, container_algorithm_internal::c_end(c));
  return num;
}
...
OutputIterator c_set_intersection(const C1& c1, const C2& c2,
  ...
  // using operator<.
  assert(::c_is_sorted(c1));
  assert(::c_is_sorted(c2));
...
OutputIterator c_set_intersection(const C1& c1, const C2& c2,
  ...
  assert(::c_is_sorted(c1, comp));
  assert(::c_is_sorted(c2, comp));
  return std::set_intersection(container_algorithm_internal::c_begin(c1),
                               container_algorithm_internal::c_end(c1),
                               container_algorithm_internal::c_begin(c2),
                               container_algorithm_internal::c_end(c2), output,
...
template <typename Sequence, typename T>
container_algorithm_internal::decay_t<T> c_accumulate(const Sequence& sequence,
                                                      T&& init) {
...
container_algorithm_internal::decay_t<T> c_inner_product(
...
inline uint64_t combineIndex(uint32_t key1, uint16_t key2, uint16_t key3) {
  return uint64_t(key1) << LSHIFT_UINT32 | uint64_t(key2) << LSHIFT_UINT16 |
         key3;
}

inline uint64_t combineIndex(uint16_t key1, uint16_t key2, uint16_t key3,
                             uint16_t key4) {
  return uint64_t(key1) << LSHIFT_UINT48 | uint64_t(key2) << LSHIFT_UINT32 |
