...
template <class T> class spp_allocator
{
  public:
    ...
    template <class U> spp_allocator(const spp_allocator<U> &o) : _space(o.getSpace()) {}

    template <class U> spp_allocator &operator=(const spp_allocator<U> &o)
    {
        if (&o != this)
            _space = o.getSpace();
        return *this;
...
template <class T> inline bool operator==(const spp_::spp_allocator<T> &a, const spp_::spp_allocator<T> &b)
{
...
namespace std
{
template <class T> inline void swap(spp_::spp_allocator<T> &a, spp_::spp_allocator<T> &b)
{
    a.swap(b);
}
} // namespace std
...
{
    return (((size_t)p & spp_chunk_align_mask) == 0) ? 0 : ((SPP_MALLOC_ALIGNMENT - ((size_t)p & spp_chunk_align_mask)) & spp_chunk_align_mask);
}
...
class malloc_state
{
  public:
    ...
    SPP_IMPL void internal_inspect_all(void (*handler)(void *start, void *end, size_t used_bytes, void *callback_arg), void *arg);
  ...
  private:
    ...
    {
        return (i == NTREEBINS - 1) ? 0 : ((spp_size_t_bitsize - 1) - ((i >> 1) + TREEBIN_SHIFT - 2));
    }

    // The size of the smallest chunk held in bin with index i
    static bindex_t minsize_for_tree_index(bindex_t i)
    {
        return ((size_t)1 << ((i >> 1) + TREEBIN_SHIFT)) | (((size_t)(i & 1)) << ((i >> 1) + TREEBIN_SHIFT - 1));
    }
...
{
    return (malloc_state *)(((mchunkptr)((char *)(p) + (p->chunksize())))->prev_foot ^ mparams._magic);
}
#endif
...
{
    return align_offset(chunk2mem((void *)0)) + pad_request(sizeof(struct malloc_segment)) + MIN_CHUNK_SIZE;
}
...
int malloc_params::_init()
{
    ...
    if (_magic == 0)
    {
...
#else
        {
            ...
            gsize = ((SPP_DEFAULT_GRANULARITY != 0) ? SPP_DEFAULT_GRANULARITY : system_info.dwAllocationGranularity);
        }
#endif
        ...
        */
        if ((sizeof(size_t) != sizeof(char *)) || (spp_max_size_t < MIN_CHUNK_SIZE) || (sizeof(int) < 4) || (SPP_MALLOC_ALIGNMENT < (size_t)8U) ||
            ((SPP_MALLOC_ALIGNMENT & (SPP_MALLOC_ALIGNMENT - 1)) != 0) || ((MCHUNK_SIZE & (MCHUNK_SIZE - 1)) != 0) || ((gsize & (gsize - 1)) != 0) ||
            ((psize & (psize - 1)) != 0))
            SPP_ABORT;
        _granularity    = gsize;
        _page_size      = psize;
...
void malloc_state::do_check_tree(tchunkptr t)
{
    ...
    do
    {
        ...
        else
        {
            ...
            head = u;
            assert(u->_parent != u);
            assert(u->_parent->_child[0] == u || u->_parent->_child[1] == u || *((tbinptr *)(u->_parent)) == u);
            if (u->_child[0] != 0)
            {
...
size_t malloc_state::traverse_and_check()
{
    ...
    if (is_initialized())
    {
        ...
        while (s != 0)
        {
            ...
            mchunkptr lastq = 0;
            assert(q->pinuse());
            while (segment_holds(s, q) && q != _top && q->_head != FENCEPOST_HEAD)
            {
                sum += q->chunksize();
                if (q->is_inuse())
                {
...
void malloc_state::unlink_small_chunk(mchunkptr p, size_t s)
{
    ...
    if (rtcheck(F == smallbin_at(I) || (ok_address(F) && F->_bk == p)))
    {
        if (B == F)
            clear_smallmap(I);
        else if (rtcheck(B == smallbin_at(I) || (ok_address(B) && B->_fd == p)))
        {
            F->_bk = B;
            B->_fd = F;
        }
...
void malloc_state::unlink_large_chunk(tchunkptr X)
{
    ...
    {
        tchunkptr *RP;
        if (((R = *(RP = &(X->_child[1]))) != 0) || ((R = *(RP = &(X->_child[0]))) != 0))
        {
            tchunkptr *CP;
            while ((*(CP = &(R->_child[1])) != 0) || (*(CP = &(R->_child[0])) != 0))
                R = *(RP = CP);
...
mchunkptr malloc_state::mmap_resize(mchunkptr oldp, size_t nb, int flags)
{
    ...
    if (oldsize >= nb + sizeof(size_t) && (oldsize - nb) <= (mparams._granularity << 1))
        return oldp;
    else
    {
        ...
        char *cp         = (char *)SPP_CALL_MREMAP((char *)oldp - offset, oldmmsize, newmmsize, flags);
        if (cp != cmfail)
        {
    ...
    */
    ...
    if (tbase != cmfail)
    {
        ...
        else
        {
            ...
            if (sp != 0 && !sp->is_extern_segment() && (sp->_sflags & USE_MMAP_BIT) == mmap_flag && segment_holds(sp, _top))
            {
                // append
                sp->_size += tsize;
            ...
            else
            {
                ...
                while (sp != 0 && sp->_base != tbase + tsize)
                    sp = (SPP_NO_SEGMENT_TRAVERSAL) ? 0 : sp->_next;
                if (sp != 0 && !sp->is_extern_segment() && (sp->_sflags & USE_MMAP_BIT) == mmap_flag)
                {
...
size_t malloc_state::release_unused_segments()
{
    ...
    // Reset check counter
    _release_checks = (((size_t)nsegs > (size_t)SPP_MAX_RELEASE_CHECK_RATE) ? (size_t)nsegs : (size_t)SPP_MAX_RELEASE_CHECK_RATE);
    return released;
}
    ...
    if (pad < MAX_REQUEST && is_initialized())
    {
        ...
        if (_topsize > pad)
        {
            ...
            if (!sp->is_extern_segment())
            {
                ...
                {
                    if (SPP_HAVE_MMAP && sp->_size >= extra && !has_segment_link(sp))
                    {
...
mchunkptr malloc_state::try_realloc_chunk(mchunkptr p, size_t nb, int can_move)
{
    ...
    if (rtcheck(ok_address(p) && ok_inuse(p) && ok_next(p, next) && ok_pinuse(next)))
    {
        if (p->is_mmapped())
        ...
        if (mem != 0)
        {
            ...
            if ((((size_t)(mem)) & (alignment - 1)) != 0)
            {
                ...
                /*
                  ...
                  possible.
                */
                char *br        = (char *)mem2chunk((void *)(((size_t)((char *)mem + alignment - 1)) & -alignment));
...
*/
void **malloc_state::ialloc(size_t n_elements, size_t *sizes, int opts, void *chunks[])
{
...
void malloc_state::internal_inspect_all(void (*handler)(void *start, void *end, size_t used_bytes, void *callback_arg), void *arg)
{
    if (is_initialized())
    {
        mchunkptr top = top;
...
SPP_API mspace create_mspace(size_t capacity, int locked)
{
    ...
    {
        size_t rs    = ((capacity == 0) ? mparams._granularity : (capacity + top_foot_size() + msize));
...
SPP_API size_t destroy_mspace(mspace msp)
{
    ...
    if (ms->ok_magic())
    {
        ...
        while (sp != 0)
        {
            ...
            sp = sp->_next;
            if ((flag & USE_MMAP_BIT) && !(flag & EXTERN_BIT) && SPP_CALL_MUNMAP(base, size) == 0)
                freed += size;
        }
    }
    else
...
#if SPP_MALLOC_INSPECT_ALL
SPP_API void mspace_inspect_all(mspace msp, void (*handler)(void *start, void *end, size_t used_bytes, void *callback_arg), void *arg)
{
    mstate ms = (mstate)msp;
    if (ms->ok_magic())
