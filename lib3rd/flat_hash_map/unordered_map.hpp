...
class sherwood_v10_table : private EntryAlloc, private Hasher, private Equal, private BucketAllocator
{
  ...
  public:
    ...
    sherwood_v10_table(size_type bucket_count, const ArgumentAlloc &alloc) : sherwood_v10_table(bucket_count, ArgumentHash(), ArgumentEqual(), alloc)
    {
    }
    ...
    sherwood_v10_table(sherwood_v10_table &&other) noexcept
        : EntryAlloc(std::move(other)), Hasher(std::move(other)), Equal(std::move(other)), BucketAllocator(std::move(other)),
    ...
    {
        templated_iterator() {}
        templated_iterator(EntryPointer element, EntryPointer *bucket) : current_element(element), current_bucket(bucket) {}
  ...
  private:
    ...
    template <typename... Args> SKA_NOINLINE(std::pair<iterator, bool>) emplace_new_key(EntryPointer *bucket, Args &&...args)
    {
        using std::swap;
        if (is_full())
        {
...
{
    using Table =
        detailv10::sherwood_v10_table<T, T, H, detailv10::functor_storage<size_t, H>, E, detailv10::functor_storage<bool, E>, A,
                                      typename std::allocator_traits<A>::template rebind_alloc<detailv10::sherwood_v10_entry<T, A>>,
                                      typename std::allocator_traits<A>::template rebind_alloc<
