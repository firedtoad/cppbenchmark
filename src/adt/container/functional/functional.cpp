...
int CallFunctionRef(const llvm::function_ref<void()> &fun)
{
    fun();
    return 0;
}
...
static void BM_FunctionRefStack(benchmark::State &state)
{
    int *p  = new int;
    std::unordered_map<int, int> umap;
    for (auto _ : state)
...
BENCHMARK(BM_FunctionRefStack);

static void BM_FunctionRefHeap(benchmark::State &state)
{
    int *p  = new int;
    int *p1 = new int;
    std::unordered_map<int, int> umap;
