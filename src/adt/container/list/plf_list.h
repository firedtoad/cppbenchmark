...
#define PLF_BLOCK_MIN                                                                                                                                \
    static_cast<group_size_type>(                                                                                                                    \
        (sizeof(node) * 8 > (sizeof(*this) + sizeof(group)) * 2) ? 8 : (((sizeof(*this) + sizeof(group)) * 2) / sizeof(node)) + 1)
...
#define PLF_IS_ALWAYS_EQUAL_SUPPORT
    ...
    ((defined(__clang__) && ((__clang_major__ == 3 && __clang_minor__ == 9) || __clang_major__ > 3)) || (defined(__GNUC__) && __GNUC__ >= 7) ||      \
     (!defined(__clang__) && !defined(__GNUC__))) // assume correct C++17 implementation for non-gcc/clang compilers
#define PLF_CONSTEXPR constexpr
#else
#define PLF_CONSTEXPR
#endif
    ...
    ((defined(__clang__) && (__clang_major__ >= 13)) || (defined(__GNUC__) && __GNUC__ >= 10) || (!defined(__clang__) && !defined(__GNUC__)))
#define PLF_CPP20_SUPPORT
#endif
#else
...
#if defined(PLF_IS_ALWAYS_EQUAL_SUPPORT) && defined(PLF_MOVE_SEMANTICS_SUPPORT) && defined(PLF_ALLOCATOR_TRAITS_SUPPORT) &&                          \
    (__cplusplus >= 201703L || (defined(_MSVC_LANG) && (_MSVC_LANG >= 201703L)))
#define PLF_NOEXCEPT_MOVE_ASSIGN(the_allocator)                                                                                                      \
    noexcept(std::allocator_traits<the_allocator>::propagate_on_container_move_assignment::value ||                                                  \
             std::allocator_traits<the_allocator>::is_always_equal::value)
#define PLF_NOEXCEPT_SWAP(the_allocator)                                                                                                             \
    noexcept(std::allocator_traits<the_allocator>::propagate_on_container_swap::value || std::allocator_traits<the_allocator>::is_always_equal::value)
#else
#define PLF_NOEXCEPT_MOVE_ASSIGN(the_allocator)
#define PLF_NOEXCEPT_SWAP(the_allocator)
#endif
...
#define PLF_CONSTRUCT(the_allocator, allocator_instance, location, ...)                                                                              \
    std::allocator_traits<the_allocator>::construct(allocator_instance, location, __VA_ARGS__)
#else
#define PLF_CONSTRUCT(the_allocator, allocator_instance, location, data)                                                                             \
    std::allocator_traits<the_allocator>::construct(allocator_instance, location, data)
#endif
...
#define PLF_DEALLOCATE(the_allocator, allocator_instance, location, size)                                                                            \
    std::allocator_traits<the_allocator>::deallocate(allocator_instance, location, size)
#else
#ifdef PLF_VARIADICS_SUPPORT
...
template <class element_type, class element_allocator_type = std::allocator<element_type> > class list : private element_allocator_type
{
  ...
  private:
    ...
    struct node : public node_base
    {
        ...
        node(const node_pointer_type next, const node_pointer_type previous, const element_type &source) : node_base(next, previous), element(source)
        {
        }
        ...
        node(node_pointer_type const next, node_pointer_type const previous, arguments &&...parameters)
            : node_base(next, previous), element(std::forward<arguments>(parameters)...)
        {
        }
#endif
    };
    ...
    struct group : public node_allocator_type // Node memory block + metadata
    {
        ...
        group(const group_size_type group_size, node_pointer_type const previous = NULL)
            : nodes(PLF_ALLOCATE(node_allocator_type, *this, group_size, previous)), free_list_head(NULL), beyond_end(nodes + group_size),
              number_of_elements(0)
        {
        }
#else
        // This is a hack around the fact that allocator_type::construct only supports copy construction in C++03 and copy elision does not occur on
        // the vast majority of compilers in this circumstance. And to avoid running out of memory (and performance loss) from allocating the same
        // block twice, we're allocating in this constructor and moving data in the copy constructor.
        ...
        group(const group &source)
            : node_allocator_type(source), nodes(PLF_ALLOCATE(node_allocator_type, *this, source.number_of_elements, source.free_list_head)),
              free_list_head(NULL), beyond_end(nodes + source.number_of_elements), number_of_elements(0)
        {
        }
#endif

        group &
        operator=(const group &source) PLF_NOEXCEPT // Actually a move operator, used by c++03 in group_vector's remove, expand_capacity and append
        {
            nodes              = source.nodes;
    ...
    class group_vector : private node_pointer_allocator_type // Simple vector of groups + associated functions
    {
      public:
        group_pointer_type last_endpoint_group, block_pointer,
            last_searched_group; // last_endpoint_group is the last -active- group in the block. Other -inactive- (previously used, now empty of
                                 // elements) groups may be stored after this group for future usage (to reduce deallocation/reallocation of nodes).
                                 // block_pointer + size - 1 == the last group in the block, regardless of whether or not the group is active.
        size_type size;
        ...
        group_vector() PLF_NOEXCEPT : node_pointer_allocator_type(node_pointer_allocator_type()),
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
        group_vector(group_vector &&source) PLF_NOEXCEPT : last_endpoint_group(std::move(source.last_endpoint_group)),
        ...
        void expand_capacity(const size_type new_capacity) // used by add_new and append
        {
            ...
            { // Dereferencing here in order to deal with smart pointer situations ie. obtaining the raw pointer from the smart pointer
                std::memcpy(static_cast<void *>(&*block_pointer), static_cast<void *>(&*old_block),
                            sizeof(group) * size); // static_cast or reinterpret_cast necessary to deal with GCC 8 warnings
            }
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
...
#endif
            {
                // If allocator supplies non-trivial pointers it becomes necessary to destroy the group. uninitialized_copy will not work in this
                // context as the copy constructor for "group" is overriden in C++03/98. The = operator for "group" has been overriden to make the
                // following work:
        ...
        void
        initialize(const group_size_type group_size) // For adding first group *only* when group vector is completely empty and block_pointer is NULL
        {
        ...
        group_pointer_type
        get_nearest_freelist_group(const node_pointer_type location_node) PLF_NOEXCEPT // In working implementation this cannot throw
        {
            ...
            else // search for the node group which location_node is located within, using last_searched_group as a starting point and searching left
                 // and right. Try and find the closest node group with reusable erased-element locations along the way:
            {
                group_pointer_type closest_freelist_left  = (last_searched_group->free_list_head == NULL) ? NULL : last_searched_group,
                                   closest_freelist_right = (last_searched_group->free_list_head == NULL) ? NULL : last_searched_group;
                ...
                while (true)
                {
                    if (right_not_beyond_back)
                    {
                        if ((location_node < right->beyond_end) && (location_node >= right->nodes)) // location_node's group is found
                        {
                            ...
                            // Otherwise find closest group with freelist - check an equal distance on the right to the distance we've checked on the
                            // left:
                            const group_pointer_type end_group =
                                (((right + left_distance) > beyond_end_group) ? beyond_end_group : (right + left_distance - 1));
                    ...
                    if (left_not_beyond_front)
                    {
                        if ((location_node >= left->nodes) && (location_node < left->beyond_end))
                        {
                            ...
                            const group_pointer_type end_group =
                                (((left - right_distance) < block_pointer) ? block_pointer - 1 : (left - right_distance) + 1);
            ...
            // The node group which location_node is located within, is known at this point. Continue searching outwards from this group until a group
            // is found with a reusable location:
            while (true)
            {
                if (right_not_beyond_back)
                {
        ...
        {
#ifdef PLF_TYPE_TRAITS_SUPPORT
            if PLF_CONSTEXPR (std::is_trivial<group_pointer_type>::value) // if pointer type is trivial we can just copy using memcpy - faster -
                                                                          // avoids constructors/destructors etc
            {
...
#endif
            {
                const group_pointer_type swap_last_endpoint_group = last_endpoint_group, swap_block_pointer = block_pointer,
        ...
        void append(group_vector &source)
        {
            ...
            {
                std::uninitialized_copy(std::make_move_iterator(source.block_pointer), std::make_move_iterator(source.block_pointer + source.size),
                                        block_pointer + size);
            }
#endif
            else
  ...
  private:
    // Used by range-insert and range-constructor to prevent fill-insert and fill-constructor function calls mistakenly resolving to the range
    // insert/constructor
    template <bool condition, class T = void> struct plf_enable_if_c
    {
        typedef T type;
    };

    template <class T> struct plf_enable_if_c<false, T>
    {
    };
    ...
    node_pointer_type last_endpoint; // The node location which is one-past the last inserted element in the last group of the list. Is not affected
                                     // by erasures to prior elements (these are handled using the group's freelist).
    // If last_endpoint is beyond the end of a memory block it means a new group must be created upon the next insertion if prior erased nodes are not
    // available for re-use. last_endpoint == NULL means total_size is zero, but there may still be groups available due to calling clear(), reserve()
    // on an empty list, or having erased all elements in the list groups.block_pointer == NULL means an uninitialized container ie. no groups or
    // elements yet
    iterator end_iterator, begin_iterator; // Returned by begin() and end().
    // end_iterator always points to end_node. It is a convenience/optimization variable to save generating many temporary iterators from end_node
    // during functions and during end(). When the list is empty of elements, begin_iterator == end_iterator so that program loops iterating from
  ...
  public:
    ...
    list() PLF_NOEXCEPT : element_allocator_type(element_allocator_type()),
    ...
    list(const list &source)
        : element_allocator_type(source), end_node(static_cast<node_pointer_type>(&end_node), static_cast<node_pointer_type>(&end_node)),
          last_endpoint(NULL), end_iterator(static_cast<node_pointer_type>(&end_node)), begin_iterator(static_cast<node_pointer_type>(&end_node)),
          node_pointer_allocator_pair(0), node_allocator_pair(0)
    {
    ...
    list(list &&source) PLF_NOEXCEPT
        : element_allocator_type(source),
          ...
          end_iterator(static_cast<node_pointer_type>(&end_node)),
          begin_iterator((source.begin_iterator.node_pointer == source.end_iterator.node_pointer) ? static_cast<node_pointer_type>(&end_node)
    ...
    list(list &&source, const allocator_type &alloc)
        : element_allocator_type(alloc), groups(std::move(source.groups)), end_node(std::move(source.end_node)),
          last_endpoint(std::move(source.last_endpoint)), end_iterator(static_cast<node_pointer_type>(&end_node)),
          begin_iterator((source.begin_iterator.node_pointer == source.end_iterator.node_pointer) ? static_cast<node_pointer_type>(&end_node)
                                                                                                  : std::move(source.begin_iterator)),
          node_pointer_allocator_pair(source.node_pointer_allocator_pair.total_size),
          node_allocator_pair(source.node_allocator_pair.number_of_erased_nodes)
    ...
    list(const typename plf_enable_if_c<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type &first, const iterator_type &last,
  ...
  private:
    ...
    inline void add_group_if_necessary()
    {
        ...
        {
            if (static_cast<size_type>(groups.last_endpoint_group - groups.block_pointer) ==
                groups.size - 1) // ie. there are no reusable groups available at the back of group vector
            {
                groups.add_new((node_pointer_allocator_pair.total_size < PLF_BLOCK_MAX)
                                   ? static_cast<group_size_type>(node_pointer_allocator_pair.total_size)
                                   : PLF_BLOCK_MAX);
            }
            else
            {
  ...
  public:
    iterator insert(const const_iterator it, const element_type &element)
    {
        if (last_endpoint != NULL) // ie. list is not empty
        {
            ...
            {
                group_pointer_type const node_group =
                    groups.get_nearest_freelist_group((it.node_pointer != end_iterator.node_pointer) ? it.node_pointer : end_node.previous);
        ...
        else // list is empty
        {
            ...
            {
#ifdef PLF_VARIADICS_SUPPORT
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++, end_iterator.node_pointer, end_iterator.node_pointer,
                              element);
#else
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++,
                              node(end_iterator.node_pointer, end_iterator.node_pointer, element));
#endif
            }
            else
#endif
...
#ifdef PLF_MOVE_SEMANTICS_SUPPORT
    iterator
    insert(const const_iterator it,
           element_type &&element) // This is almost identical to the insert implementation above with the only change being std::move of the element
    {
        if (last_endpoint != NULL)
        {
...
#ifdef PLF_VARIADICS_SUPPORT
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint, it.node_pointer, it.node_pointer->previous,
                              std::move(element));
#else
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint,
                              node(it.node_pointer, it.node_pointer->previous, std::move(element)));
#endif
...
#ifdef PLF_VARIADICS_SUPPORT
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, selected_node, it.node_pointer, it.node_pointer->previous,
                              std::move(element));
#else
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, selected_node,
                              node(it.node_pointer, it.node_pointer->previous, std::move(element)));
#endif
...
#else
                PLF_CONSTRUCT(node_allocator_type, node_allocator_pair, last_endpoint++,
                              node(end_iterator.node_pointer, end_iterator.node_pointer, std::move(element)));
#endif
            }
            else
#endif
    ...
    template <typename... arguments>
    iterator
    emplace(const const_iterator it,
            arguments &&...parameters) // This is almost identical to the insert implementations above with the only changes being std::forward of
                                       // element parameters, removal of VARIADICS support checking, and is_nothrow_contructible
    {
        if (last_endpoint != NULL)
        {
            if (node_allocator_pair.number_of_erased_nodes == 0)
    ...
    template <typename... arguments> inline PLF_FORCE_INLINE reference emplace_back(arguments &&...parameters)
    {
        return (emplace(end_iterator, std::forward<arguments>(parameters)...)).node_pointer->element;
    }

    template <typename... arguments> inline PLF_FORCE_INLINE reference emplace_front(arguments &&...parameters)
    {
  ...
  private:
    void fill(const element_type &element, group_size_type number_of_elements, node_pointer_type const position)
    {
        ...
        groups.last_endpoint_group->number_of_elements =
            static_cast<group_size_type>(groups.last_endpoint_group->number_of_elements + number_of_elements);
        ...
        do
        {
...
#endif
            {
                ...
                catch (...)
                {
                    ...
                    groups.last_endpoint_group->number_of_elements = static_cast<group_size_type>(groups.last_endpoint_group->number_of_elements -
    ...
    template <class iterator_type> iterator range_insert(const const_iterator position, const size_type number_of_elements, iterator_type it)
    {
        ...
        while (true)
        {
            ...
            const group_size_type group_size =
                static_cast<group_size_type>(groups.last_endpoint_group->beyond_end - groups.last_endpoint_group->nodes);
  ...
  public:
    ...
    inline iterator insert(const const_iterator position,
  ...
  private:
    inline PLF_FORCE_INLINE void destroy_all_node_pointers(group_pointer_type const group_to_process,
                                                           const node_pointer_type beyond_end_node) PLF_NOEXCEPT
    {
  ...
  public:
    ...
    iterator erase(const const_iterator it) // if uninitialized/invalid iterator supplied, function could generate an exception, hence no noexcept
    {
        ...
        // find the group this element is in, starting from the last group an element to-be-erased was found in (as erasures are, for most programs,
        // likely to be closer in proximity to previous erasures):
        ...
        {
            it.node_pointer->next = NULL; // next == NULL so that destructor and other functions which linearly iterate over node memory chunks can
        ...
        else if (node_group != groups.last_endpoint_group--) // remove group (and decrement active back group)
        {
            ...
            if ((group_size == PLF_BLOCK_MAX) |
                (node_group >=
                 groups.last_endpoint_group - 1)) // Preserve only groups which are at the maximum possible size, or first/second/third-to-last active
                                                  // groups - seems to be best for performance under high-modification benchmarks
            {
        ...
        else // clear back group, leave trailing
        {
            ...
            {
                groups.last_endpoint_group =
                    groups.block_pointer; // If number of elements is zero, it indicates that this was the only group in the vector. In which case the
                                          // last_endpoint_group would be invalid at this point due to the decrement in the above else-if statement.
                                          // So it needs to be reset, as it will not be reset in the function call below.
                clear();
            }
    ...
    list &operator=(list &&source) PLF_NOEXCEPT_MOVE_ASSIGN(allocator_type)
    {
        ...
        end_node                                   = std::move(source.end_node);
        last_endpoint                              = std::move(source.last_endpoint);
        begin_iterator.node_pointer                = (source.begin_iterator.node_pointer == source.end_iterator.node_pointer)
                                                         ? end_iterator.node_pointer
                                                         : std::move(source.begin_iterator.node_pointer);
  ...
  public:
    template <class comparison_function> void sort(comparison_function compare)
    {
        ...
        node_pointer_type *const node_pointers =
            PLF_ALLOCATE(node_pointer_allocator_type, node_pointer_allocator_pair, node_pointer_allocator_pair.total_size, NULL);
        node_pointer_type *node_pointer = node_pointers;

        // According to the C++ standard, construction of a pointer (of any type) may not trigger an exception - hence, no try-catch blocks are
    ...
    void reserve(size_type reserve_amount)
    {
        ...
        { // edge case: has been filled with elements then clear()'d - some groups may be smaller than would be desired, should be replaced
            group_size_type end_group_size =
                static_cast<group_size_type>((groups.block_pointer + groups.size - 1)->beyond_end - (groups.block_pointer + groups.size - 1)->nodes);
            ...
            else
            {
                ...
                {
                    const group_size_type current_group_size =
                        static_cast<group_size_type>(groups.block_pointer->beyond_end - groups.block_pointer->nodes);
        ...
        { // Create a group at least as large as the last group - may allocate more than necessary, but better solution than creating a very small
          // group in the middle of the group vector, I think:
            const group_size_type last_endpoint_group_capacity =
                static_cast<group_size_type>(groups.last_endpoint_group->beyond_end - groups.last_endpoint_group->nodes);
            groups.add_new(
                static_cast<group_size_type>((reserve_amount < last_endpoint_group_capacity) ? last_endpoint_group_capacity : reserve_amount));
        }
    ...
    void shrink_to_fit()
    {
        if ((groups.block_pointer == NULL) |
            (node_pointer_allocator_pair.total_size == groups.element_allocator_pair.capacity)) // if list is uninitialized or full
        {
            return;
        }
        else if (node_pointer_allocator_pair.total_size == 0) // Edge case
        {
            ...
            return;
                 ...
                 last_endpoint == groups.last_endpoint_group->beyond_end) // edge case - currently no wasted space except for possible trailing groups
        {
            groups.trim_unused_groups();
            return;
        }
  ...
  public:
    ...
    {
        // Note: Because current_node->next has to be read during swapping in this process anyway, including a test to figure out whether or not a
        // given group has erased elements within it, and thus avoid per-node tests, is actually detrimental to performance according to benchmarks.
        // This is unlike sort() where current_node->next is not used in the rest of the process and avoiding per-node tests of it's value is
        // therefore beneficial in benchmarks.
  ...
  private:
    ...
    struct eq_to
    {
        ...
        explicit eq_to(const element_type store_value) : value(store_value) {}
  ...
  public:
                ...
                if (end - current_group->nodes != num_elements) // If there are erased nodes present in the group
                {
                    for (node_pointer_type current_node = current_group->nodes; current_node != end; ++current_node)
                    {
                        if (current_node->next != NULL && predicate(current_node->element)) // is not free list node and validates predicate
                        {
                            ...
                            {
                                --current_group; // As current group has been removed, subsequent groups have already shifted back by one, hence, the
                                                 // ++ to the current group in the for loop is unnecessary, and negated here
                                break;
                            }
                        }
                    }
    ...
    template <class iterator_type>
    inline void assign(const typename plf_enable_if_c<!std::numeric_limits<iterator_type>::is_integer, iterator_type>::type first,
                       const iterator_type last)
    {
        clear();
    ...
    void swap(list &source) PLF_NOEXCEPT_SWAP(allocator_type)
    {
        ...
        *this  = std::move(temp);
        ...
        const size_type swap_total_size             = node_pointer_allocator_pair.total_size,
                        swap_number_of_erased_nodes = node_allocator_pair.number_of_erased_nodes;

        last_endpoint = source.last_endpoint;
        end_node.next = begin_iterator.node_pointer =
            (source.begin_iterator.node_pointer != source.end_iterator.node_pointer) ? source.begin_iterator.node_pointer : end_iterator.node_pointer;
        end_node.previous =
            (source.begin_iterator.node_pointer != source.end_iterator.node_pointer) ? source.end_node.previous : end_iterator.node_pointer;
        end_node.previous->next = begin_iterator.node_pointer->previous = end_iterator.node_pointer;
        node_pointer_allocator_pair.total_size                          = source.node_pointer_allocator_pair.total_size;
        node_allocator_pair.number_of_erased_nodes                      = source.node_allocator_pair.number_of_erased_nodes;

        source.last_endpoint = swap_last_endpoint;
        source.end_node.next = source.begin_iterator.node_pointer =
            (swap_begin_iterator.node_pointer != end_iterator.node_pointer) ? swap_begin_iterator.node_pointer : source.end_iterator.node_pointer;
        source.end_node.previous =
            (swap_begin_iterator.node_pointer != end_iterator.node_pointer) ? swap_end_node_previous : source.end_iterator.node_pointer;
...
inline void swap(plf::list<element_type, element_allocator_type> &a, plf::list<element_type, element_allocator_type> &b)
...
inline typename plf::list<element_type, element_allocator_type>::size_type erase_if(plf::list<element_type, element_allocator_type> &container,
...
template <class element_type, class element_allocator_type>
inline typename plf::list<element_type, element_allocator_type>::size_type erase(plf::list<element_type, element_allocator_type> &container,
                                                                                 const element_type &value)
{
