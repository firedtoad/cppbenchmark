...
template <class S>
static void BenchEraseUnOrderSetInt(benchmark::State &state) {
  S s;
  s.reserve(65536);
...
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetInt,
                   robin_hood::unordered_flat_set<int>);
...
template <class S>
static void BenchEraseUnOrderSetString(benchmark::State &state) {
  S s;
...
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetString,
                   ska::bytell_hash_set<std::string>);
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetString,
                   phmap::flat_hash_set<std::string>);
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetString,
                   robin_hood::unordered_flat_set<std::string>);
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetString,
                   spp::sparse_hash_set<std::string>);
BENCHMARK_TEMPLATE(BenchEraseUnOrderSetString,
                   tsl::bhopscotch_set<std::string>);
