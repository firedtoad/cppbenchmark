...
template<class M>
static void BenchUnOrderSetInt(benchmark::State &state) {
    M m;
    m.reserve(65536);
...
template<class M>
static void BenchUnOrderSetString(benchmark::State &state) {
    M m;
    m.reserve(65536);
...
template<class M>
static void BenchCharKeySet(benchmark::State &state) {
     M m;
    std::vector<std::string> keys(65536);
    for (auto i = 0; i < 65536; i++) {
        keys[i] = std::to_string(_random() % 65536);
