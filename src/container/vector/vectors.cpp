...
BENCHMARK_TEMPLATE(BenchInsert, llvm_vecsmall::SmallVector<int, 1024>)
    ->Range(2, 1024);
...
BENCHMARK_TEMPLATE(BenchReserve, llvm_vecsmall::SmallVector<int, 1024>)
    ->Range(2, 1024);
...
BENCHMARK_TEMPLATE(BenchAccumulate, llvm::SmallVector<int, 1024>)
    ->Range(2, 1024);
BENCHMARK_TEMPLATE(BenchAccumulate, llvm_vecsmall::SmallVector<int, 1024>)
    ->Range(2, 1024);

template <typename V>
static void BenchAccumulateReverse(benchmark::State &state) {
  V v;
  v.reserve(state.range(0));
  for (auto i = 0; i < state.range(0); i++) {
    v.push_back(i);
...
BENCHMARK_TEMPLATE(BenchAccumulateReverse, llvm::SmallVector<int, 1024>)
    ->Range(2, 1024);
BENCHMARK_TEMPLATE(BenchAccumulateReverse,
                   llvm_vecsmall::SmallVector<int, 1024>)
    ->Range(2, 1024);
