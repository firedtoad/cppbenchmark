...
template <typename FormatContext, typename OutputIt, typename Rep, typename Period> struct chrono_formatter
{
    ...
    {
        constexpr auto num_fractional_digits = count_fractional_digits(Duration::period::num, Duration::period::den);
        ...
        if (std::ratio_less<typename subsecond_precision::period, std::chrono::seconds::period>::value)
        {
            *out++ = '.';
            ...
            using sec =
                conditional_t<std::is_same<typename Duration::rep, long double>::value, std::chrono::duration<long double>, std::chrono::seconds>;
