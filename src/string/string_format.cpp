...
void StringFormat(T &t, std::string &str) {
    S oss(str);
    oss << t;
    str = oss.str();
}
...
template<typename S, typename T>
static void BenchStreamStringFormat(benchmark::State &state) {
    std::string r;
    for (auto _ : state) {
...
template<typename S, typename T>
static void BenchStreamStringFormatF(benchmark::State &state) {
    std::string r;
    for (auto _ : state) {
...
template<typename T>
static void BenchSnprintf(benchmark::State &state) {
    for (auto _ : state) {
        char buff[256]{};
        auto r = snprintf(buff, 256, "%d", tag);
        benchmark::DoNotOptimize(r);
    }
}
...
template<typename T>
static void BenchSnprintfFloat(benchmark::State &state) {
    for (auto _ : state) {
        char buff[256]{};
        auto r = snprintf(buff, 256, "%f", tag);
        benchmark::DoNotOptimize(r);
    }
}
...
template<typename T>
static void BenchBoostStringFormat(benchmark::State &state) {
    for (auto _ : state) { ;
...
template<typename T>
static void BenchBoostStringFormatf(benchmark::State &state) {
    for (auto _ : state) { ;
...
template<typename T>
static void BenchFmtStrFormat(benchmark::State &state) {
    for (auto _ : state) {
...
template<typename T>
static void BenchFmtStrFormatf(benchmark::State &state) {
    for (auto _ : state) {
