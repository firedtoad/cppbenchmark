...
template <typename T> inline void futex_wake_single(std::atomic<T> &to_wake)
{
#ifdef _WIN32
...
template <typename T> inline void futex_wake_all(std::atomic<T> &to_wake)
{
#ifdef _WIN32
...
struct semaphore
{
    ...
    explicit semaphore(LONG initial_amount) : windows_semaphore(CreateSemaphoreA(nullptr, initial_amount, std::numeric_limits<LONG>::max(), nullptr))
    {
    }
    ~semaphore()
    {
...
struct semaphore_custom
{
  ...
  public:
    explicit semaphore_custom(ptrdiff_t desired = 0) : count(desired) {}
...
template <int SpinNoOpCount, int SpinPauseCount, bool ResetCountAfterYield> struct parameterized_spinlock
{
    void lock()
    {
        int spin_count = 0;
...
template <typename State> struct ThreadedBenchmarkRunner
{
    ThreadedBenchmarkRunner(State *state, int num_threads) : state(state)
    {
...
template <typename State> struct ThreadedBenchmarkRunnerMultipleTimes
{
    template <typename... StateArgs> ThreadedBenchmarkRunnerMultipleTimes(int num_runners, int num_threads, StateArgs &&...state_args)
    {
...
template <typename T> struct ContendedMutexRunner
{
    size_t num_loops = 1;
    size_t sum       = 0;
    size_t expected_sum;
    ...
    explicit ContendedMutexRunner(int num_threads, size_t num_loops) : num_loops(num_loops), expected_sum(num_threads * num_loops) {}
...
template <typename T> struct ContendedMutexRunnerSized
{
    size_t num_loops = 1;
    ...
    explicit ContendedMutexRunnerSized(int num_threads, size_t num_loops, size_t num_sums)
        : num_loops(num_loops), sums(num_sums), expected_sum(num_threads * num_loops)
    {
    }
...
#ifdef _WIN32
template <typename Randomness> uint64_t UniformRandom(uint64_t max, Randomness &random)
{
...
template <typename T> struct ManyContendedMutexRunner
{
    struct OneMutexAndState
    {
        explicit OneMutexAndState(size_t num_sums) : sums(num_sums) {}
        std::vector<size_t> sums;
        T mutex;
    };
    ...
    struct ThreadState
    {
        template <typename Seed> ThreadState(Seed &random_seed, size_t num_loops, std::deque<OneMutexAndState> &state) : randomness(random_seed)
        {
    ...
    explicit ManyContendedMutexRunner(int num_threads, size_t num_loops, size_t num_sums) : expected_sum(num_loops)
    {
        for (int i = 0; i < num_threads; ++i)
        {
...
template <typename T> void BenchmarkContendedMutex(benchmark::State &state)
{
...
template <typename T> struct ContendedMutexRunnerMoreIdle
{
    size_t sum          = 0;
    size_t num_loops    = 1;
    ...
    explicit ContendedMutexRunnerMoreIdle(int num_threads, size_t num_loops) : num_loops(num_loops), expected_sum(num_threads * num_loops) {}
...
template <typename T> void BenchmarkContendedMutexMoreIdle(benchmark::State &state)
{
...
template <typename T, size_t N> struct TopNHeap
{
    ...
    template <size_t ON> void merge(const TopNHeap<T, ON> &to_merge)
    {
...
template <typename T> struct LongestWaitRunner
{
    ...
    explicit LongestWaitRunner(int, size_t num_loops) : num_loops(num_loops)
    {
    ...
    void run_benchmark(int)
    {
        for (size_t i = num_loops; i != 0; --i)
        {
            ...
            size_t wait_time_nanos =
                std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - time_before).count();
...
template <typename T> void BenchmarkLongestWait(benchmark::State &state)
{
    static constexpr size_t num_loops = 1024 * 16;
...
template <typename T> struct LongestIdleRunner
{
    ...
    explicit LongestIdleRunner(int, size_t num_loops) : num_loops(num_loops)
    {
...
template <typename T> void BenchmarkLongestIdle(benchmark::State &state)
{
    static constexpr size_t num_loops = 1024 * 16;
...
template <typename T> struct ContendedMutexMoreWork
{
    ...
    explicit ContendedMutexMoreWork(int num_threads, size_t num_loops) : num_loops(num_loops)
    {
...
template <typename T> void BenchmarkContendedMutexMoreWork(benchmark::State &state)
{
    static constexpr size_t num_loops = 1024;
...
template <typename T> struct ThroughputRunner
{
    std::chrono::high_resolution_clock::time_point *done_time;
    size_t sum = 0;
    T mutex;

    explicit ThroughputRunner(int, std::chrono::high_resolution_clock::time_point *done_time) : done_time(done_time) {}
...
template <typename T> void BenchmarkThroughput(benchmark::State &state)
{
    ...
    runner.start_run();
    runner.finish_run();
    size_t sum = std::accumulate(runner.runners.begin(), runner.runners.end(), size_t(0), [](size_t l, const auto &r) { return l + r.state.sum; });
    runner.shut_down();
    state.counters["Throughput"] = sum;
}

template <typename T> struct ThroughputRunnerMultipleMutex
{
    std::chrono::high_resolution_clock::time_point *done_time;
    ...
    explicit ThroughputRunnerMultipleMutex(int, std::chrono::high_resolution_clock::time_point *done_time) : done_time(done_time) {}
...
template <typename T> void BenchmarkThroughputMultipleMutex(benchmark::State &state)
{
    std::chrono::high_resolution_clock::time_point end_time = std::chrono::high_resolution_clock::now();
    ...
    runner.start_run();
    runner.finish_run();
    size_t sum = std::accumulate(runner.runners.begin(), runner.runners.end(), size_t(0),
                                 [](size_t l, const auto &r) {
                                     return l + std::accumulate(std::begin(r.state.sums), std::end(r.state.sums), size_t(0),
                                                                [](size_t l, const auto &r) { return l + r.sum; });
                                 });
    runner.shut_down();
    state.counters["Throughput"] = sum;
}

template <typename T> void BenchmarkDemingWS(benchmark::State &state)
{
...
template <typename Mutex> struct alignas(64) shmemq
{
    shmemq(unsigned long max_count, unsigned int element_size) : element_size(element_size), max_size(max_count * element_size)
    {
...
template <typename T, typename State> void BenchmarkShmemq(State &state)
{
