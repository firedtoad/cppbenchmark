...
int main(int argc, char *argv[])
{
    ...
    PrintContainer(std::set<uint8_t>{}, std::set<uint16_t>{}, std::set<uint32_t>{}, std::set<uint64_t>{});
    PrintContainer(std::map<uint8_t, uint8_t>{}, std::map<uint16_t, uint16_t>{}, std::map<uint32_t, uint32_t>{}, std::map<uint64_t, uint64_t>{});
    return 0;
}
