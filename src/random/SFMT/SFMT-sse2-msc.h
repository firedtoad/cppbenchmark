...
static void gen_rand_array(sfmt_t *sfmt, w128_t *array, int size)
{
    ...
    for (; i < SFMT_N; i++)
    {
        array[i].si = mm_recursion(pstate[i].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1          = r2;
        r2          = array[i].si;
    }
    for (; i < size - SFMT_N; i++)
    {
        array[i].si = mm_recursion(array[i - SFMT_N].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1          = r2;
        r2          = array[i].si;
    }
...
static void gen_rand_array(sfmt_t *sfmt, w128_t *array, int size)
{
    ...
    for (; i < SFMT_N; i++)
    {
        mm_recursion(array[i].si, pstate[i].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1 = r2;
        r2 = array[i].si;
    }
    for (; i < size - SFMT_N; i++)
    {
        mm_recursion(array[i].si, array[i - SFMT_N].si, array[i + SFMT_POS1 - SFMT_N].si, r1, r2);
        r1 = r2;
        r2 = array[i].si;
    }
    for (j = 0; j < 2 * SFMT_N - size; j++)
