...
AStar::uint AStar::Node::getScore() const { return S; }

void AStar::Node::updateScore() { S = G + H; }
...
AStar::CoordinateList AStar::Generator::findPath(const Vec2i &source_,
                                                 const Vec2i &target_) {
...
AStar::CoordinateList AStar::Generator::findPathStack(const Vec2i &source_,
                                                      const Vec2i &target_) {
  if (detectCollision(source_) || detectCollision(target_)) {
    return {};
  }
...
AStar::Node *AStar::Generator::findNodeOnMap(CoordMap &nodes_,
                                             const Vec2i &coordinates_) {
...
void AStar::Generator::setWeight(float epsilon_) { epsilon = epsilon_; }
...
float AStar::Relaxer::static_weight(float e, int n, int len) { return e; }
...
AStar::Vec2i AStar::Heuristic::getDelta(const Vec2i &source_,
                                        const Vec2i &target_) {
  return {abs(source_.x - target_.x), abs(source_.y - target_.y)};
}

AStar::uint AStar::Heuristic::manhattan(const Vec2i &source_,
                                        const Vec2i &target_) {
  const auto &delta = getDelta(source_, target_);
  return static_cast<uint>(10 * (delta.x + delta.y));
}

AStar::uint AStar::Heuristic::euclidean(const Vec2i &source_,
                                        const Vec2i &target_) {
  const auto &delta = getDelta(source_, target_);
  return static_cast<uint>(10 * sqrt(pow(delta.x, 2) + pow(delta.y, 2)));
}

AStar::uint AStar::Heuristic::octagonal(const Vec2i &source_,
                                        const Vec2i &target_) {
  const auto &delta = getDelta(source_, target_);
