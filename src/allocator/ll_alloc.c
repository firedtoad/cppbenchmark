...
static void big_freed(void *p, size_t size)
{
    ...
    for (i = 0; i < LEAK_MAX; i++)
    {
        ...
        if (big_leak[i].size != size)
            errx(1, "big alloc size missmatch\n");
        big_leak[i].p = NULL;
        mutex_unlock(&l_lock);
...
static void test_queue(atls *tl)
{
    ...
    for (q = tl->head; q; q = q->next)
    {
        ...
        if (((uintptr_t)q) & 15)
            errx(1, "incoming queue corrupted\n");

        b = CONTAINER(btree, data, q);

        if (un_used(b))
            errx(1, "queue element marked as unused\n");
    }
}
#else
...
#ifdef DEBUG_ALLOC_SLOW
    if (un_used(b))
        errx(1, "fast_free() needs used node\n");
    if (b->s.size != ms)
        errx(1, "fast_free size wrong\n");
#endif
...
static void test_node(atls *tl, btree *b)
{
    ...
    if (tl != get_tls())
        errx(1, "tls incorrect\n");
    if (m->tail != &tl->tail)
        errx(1, "node owner wrong\n");
}
...
static void test_fast_lists(atls *tl)
{
    ...
    for (i = 0; i < NUM_FL; i++)
    {
        ...
        scan_slist(p, f)
        {
            ...
            if (un_used(b))
                errx(1, "Unused element in fast list\n");
            check_sep(b);

            j = size2fl(b->s.size);
            if ((i != j) && (i != j - 1))
                errx(1, "Fast element in wrong bin\n");

            if (!(((uintptr_t)b ^ (uintptr_t)tl) & ~(PAGESIZE - 1)))
                errx(1, "tls on fast list!\n");
            // if (f == tl->head) errx(1, "queue head in fast list\n");

            if (f->next == f)
                errx(1, "fast list loop\n");
        }
    }
}
#else
...
static void init_handler(void)
{
    ...
    if (pthread_key_create)
    {
        ...
        if (res)
            errx(1, "pthread_key_create() failed\n");
    }
    ...
    if (d_list)
    {
        ...
        if (d_list)
        {
            ...
            if (pthread_setspecific)
                pthread_setspecific(death_key, tl);
...
static patch patch_list[] = {
    PATCH_FUNC(free),    PATCH_FUNC(malloc),       PATCH_FUNC(calloc),          PATCH_FUNC(realloc),        PATCH_FUNC(_msize),
    PATCH_FUNC(_expand), PATCH_FUNC(_free_nolock), PATCH_FUNC(_realloc_nolock), PATCH_FUNC(_calloc_nolock), PATCH_FUNC(_msize_nolock),
    {NULL, NULL}};
...
static void test_in_btree(atls *tl, btree *b)
{
    ...
    if (!b_leaf(b))
        errx(1, "Unused btree object that is not a leaf\n");

    while (b->parent)
    {
        bp = b->parent;

        if (b_ptr(bp, b_pindex(b)) != b)
            errx(1, "Parent doesn't own %p\n", (void *)b);
        if (!bp->bsize[b_pindex(b)])
            errx(1, "Parent link broken\n");
...
static void test_blocks(atls *tl)
{
    ...
    if (tl->bl.next->prev != &tl->bl)
        errx(1, "Block list corrupt\n");
    ...
    scan_list(&tl->bl, d)
    {
        ...
        {
            if (b < &m->b)
                errx(1, "Node before block start!\n");
            ...
            else
            {
                ...
                if (shift(b, -(uintptr_t)(b->s.bs_offset & ~15)) != m)
                    errx(1, "Block back link broken\n");

                if (read_bs(b) != m)
                    errx(1, "Block start corrupted\n");
            }

            check_sep(b);

            if ((size > QS_MAX) && un_used(b))
                test_in_btree(tl, b);
        }
    }
}
...
static unsigned test_btree_aux(atls *tl, btree *b, unsigned lsize)
{
    ...
    while (n)
    {
        ...
        if (bn->parent != b)
            errx(1, "Btree parent incorrect\n");
        if (b_pindex(bn) != n)
            errx(1, "Btree p_index incorrect\n");

        if (b_mask(b) & (1 << (n - 1)))
            errx(1, "Used btree node marked free\n");
        msk -= (1 << (n - 1));
        ...
        if (b->bsize[n] < ssize)
            errx(1, "Btree size misordered\n");
        ssize = b->bsize[n] & ~0xff;

        if (b_leaf(bn))
        {
            if (bn->s.size * 16 != ssize)
                errx(1, "Btree leaf size wrong\n");

            if (!un_used(bn))
                errx(1, "Btree leaf marked used!\n");
        }
        else if (!is_slab(bn) && un_used(bn))
            errx(1, "Btree node marked unused!\n");

        if (b_prev(b, b_next(b, n)) != n)
            errx(1, "prev link broken\n");
    ...
    if (msk != b_mask(b))
        errx(1, "Btree free mask missmatch\n");

    if (b->parent && (i <= 3))
        errx(1, "Btree has too few children\n");
...
{
    if ((tl->b_hgt > 100) || (tl->b_cnt > 100))
        errx(1, "btree height corrupt\n");
...
#ifdef DEBUG_ALLOC_SLOW
    if (!bl->parent || !br->parent)
        errx(1, "Trying to merge heap top\n");
    if (bl->parent != br->parent)
        errx(1, "Trying to merge two nodes with different parents\n");
#endif
...
static void btree_node_del(atls *tl, btree *b, int loc)
{
    ...
    if (b_next(b, b_next(b, b_next(b, b_start(b)))))
        return;
...
use_btree:
    ...
    while (tl->b_hgt > tl->b_cnt)
    {
        ...
        /* As many as required */
        if (num > tl->b_hgt - tl->b_cnt)
            num = tl->b_hgt - tl->b_cnt;
...
static void test_double_lists(atls *tl)
{
    ...
    for (i = 1; i < NUM_QS; i++)
    {
        if (tl->qs[i].next->prev != &tl->qs[i])
            errx(1, "First double link broken\n");
        ...
        scan_list_safe(&tl->qs[i], d, dn)
        {
            ...
            if (!un_used(b))
                errx(1, "False used\n");
            if (b->s.size != (i + 1) * 16)
                errx(1, "Wrong sized double link\n");
            if (b->s.bs_offset & FLG_SIZE8)
                errx(1, "flag size wrong\n");

            if (dn->prev != d)
                errx(1, "Back-link broken\n");
        }
    }

    if (tl->q_mask & (1ULL << 63))
        errx(1, "double list last bit set\n");
}
...
static void test_small_list(atls *tl)
{
    ...
    if (small_prev(small_next(q8)) != q8)
        errx(1, "First link broken\n");
...
#ifdef DEBUG_ALLOC
    if (offset - 1 + total - (uintptr_t)sb > SLABSIZE)
        errx(1, "slab overflow\n");
#endif
...
static void *slab_alloc(atls *tl, size_t size)
{
    ...
    if (dlist_empty(slab))
        return slab_alloc_nolist(size, slab, tl);
...
static void *slab_alloc_safe(atls *tl, size_t size)
{
    ...
    if (dlist_empty(slab) && !slab_create(tl, slab, size))
        return NULL;
...
static void slab_merge(atls *tl1, atls *tl2)
{
    ...
    if (tl2->slab_chunk)
        slab_free_chunk(tl1, tl2->slab_chunk);
}
...
static btree *split_node_rhs(atls *tl, btree *b, size_t t_size, size_t msize)
{
    ...
    if (t_size != b->s.size)
        errx(1, "size missmatch\n");
    if (msize > t_size)
        errx(1, "too big to fit in split\n");
    check_sep(b);
#endif
...
static void merge_node_aux(atls *tl, btree *bl)
{
    ...
    /* Is block empty? */
    if ((bl->s.bs_offset >= HEADERSIZE) || br->s.size || (msize * 4 > tl->a_alloced))
        goto save;
...
#ifdef DEBUG_ALLOC_SLOW
    if (msize & (PAGESIZE - 1))
        errx(1, "big block size incorrect\n");
#endif
...
#define SAVE_REG(N, V) asm volatile("movq %0, %%xmm" #N ::"mr"(V))

#define RESTORE_REG(N, V) asm volatile("movq %%xmm" #N ", %0" : "=r"(V))
...
static void *block_alloc(atls *tl, size_t size)
{
    ...
    b = btree_get(tl, size);
    if (b)
        return split_node(tl, b, b->s.size, size);
...
void *PREFIX(malloc)(size_t size)
{
    ...
    if (!tl)
        return malloc_aux(size);

    test_all(tl);

    if (likely(size <= SB_MAX))
        return slab_alloc(tl, size);

    /* Prevent overflow bug in sep_align() below */
    if (unlikely(size > BTMALLOC))
        return big_alloc(tl, size);
...
static noinline void *realloc_aux2(void *p, size_t size, atls *tl)
{
...
#ifdef DEBUG_ALLOC
    if (un_used(b))
        errx(1, "Realloc of unmalloced pointer %p\n", p);
#endif
    ...
    if (!msize)
    {
        ...
        if ((old_size - offset <= size * 2) && (old_size - offset >= size))
            return p;
        ...
        if (size >= BTMALLOC)
        {
            ...
            if (size < old_size)
            {
                 ...
                 */
                if (VirtualAlloc(shift(ps, size), old_size - size, MEM_RESET, PAGE_NOACCESS))
                {
                    /* Update size */
                    *ps = size;
                }
    ...
    else
    {
        ...
        /* Don't bother resizing shrinks that are more than half the allocated size */
        if ((old_size <= size * 2) && (old_size >= size))
            return p;
        ...
        if (m->tail == &tl->tail)
        {
            ...
            /* Easy case */
            if (size <= msize)
                return split_node(tl, b, msize, size);
...
static void test_align(size_t align, void *p)
{
    ...
    if (align && (x & (align - 1)))
        errx(1, "Incorrect alignment of pointer\n");
}
#else
...
void *aligned_alloc(size_t align, size_t size)
{
    ...
    if (!tl)
        return aligned_alloc_aux(align, size);
    ...
    while (1)
    {
        ...
        /* We need more space, so try to free memory. */
        if (scan_queue(tl, &tl->head, rsize))
            continue;
...
#ifdef DEBUG_ALLOC_SLOW
    if (rsize > b->s.size)
        errx(1, "node received is too small\n");
#endif
...
static void **ialloc(atls *tl, size_t n, size_t *sizes, void **chunks, int clear)
{
    ...
    while (1)
    {
        ...
        if (total_size < QS_MAX)
            p = slow_alloc(tl, total_size);
        if (p)
            break;

        /* Too large to allocate normally */
        if (total_size >= BTMALLOC)
        ...
        /* We need more space, so try to free memory. */
        if (scan_queue(tl, &tl->head, total_size))
            continue;
    ...
    if (clear && !tl->callocable)
        memset(p, 0, total_size - 8);
...
void **independent_calloc(size_t n, size_t size, void **chunks)
{
    ...
    if (!tl)
        return ialloc_aux(n, &size, chunks, 1);
...
void **independent_comalloc(size_t n, size_t *sizes, void **chunks)
{
    ...
    if (!tl)
        return ialloc_aux(n, sizes, chunks, 0);
