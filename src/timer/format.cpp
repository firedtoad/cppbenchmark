...
BENCHMARK_TEMPLATE(BenchAbSixDigitsToBuffer,
                   float);
...
template<typename T>
static void BenchFollyTo(benchmark::State &state) {
    for (auto _ : state) {
...
template<typename T>
static void BenchFollyTof(benchmark::State &state) {
    for (auto _ : state) {
        auto r = folly::to<std::string>(tagf);
        benchmark::DoNotOptimize(r);
    }
}
BENCHMARK_TEMPLATE(BenchFollyTof,
                   float);

template<typename T>
static void BenchFollyToInt(benchmark::State &state) {
    for (auto _ : state) {
...
template<typename T>
static void BenchFollyToFloat(benchmark::State &state) {
    for (auto _ : state) {
        auto r = folly::to<T>(stagf);
        benchmark::DoNotOptimize(r);
    }
}
BENCHMARK_TEMPLATE(BenchFollyToFloat,
float);
...
BENCHMARK_TEMPLATE(BenchAbSimpleToFloat,
                   float);
